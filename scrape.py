...
def parsero(url,r):
    ...
    iframe_src=[url+i["src"] for i in soup.find_all("iframe",src=True)]            #Finding the related iframe files
    js_src=[url+i["src"] for i in soup.find_all("script",src=True)]            #Finding the related javascript files   
    ...
    if(len(js_src)):
        ...
        for jrr in js_src:                                      #Looping through the javascript files serially
            if(jrr not in url):
                jsparser_links(jrr,r)                               #Function called for the related javascript files parsing
...
def main():
    ...
    global js
    slno=1
    html=dict.fromkeys(['Slno','URL','Length_of_document','No_of_iframes','No_of_iframelinks','No_of_jslinks',
                        'No_of_hyperlinks','Types_of_tags','Unsymmetrical_tags','Symmetrical_tags','Hidden_objects',
                        'No_of_concatenation','Category'],0)
    js=dict.fromkeys(['Slno','URL','Length_of_document','No_of_iframes','Keywords','escape','unescape','link','execo',
                      'search','eval','decode','encode','concat','Category'],0)
    
    #Defining the dataframes for the html and javascript features
    df_html=pd.DataFrame(columns=['Slno','URL','Length_of_document','No_of_iframes','No_of_iframelinks','No_of_jslinks',
                                  'No_of_hyperlinks','Types_of_tags','Unsymmetrical_tags','Symmetrical_tags','Hidden_objects',
                                  'No_of_concatenation','Category'])
    df_js=pd.DataFrame(columns=['Slno','URL','Length_of_document','No_of_iframes','Keywords','escape','unescape','link','execo',
                                'search','eval','decode','encode','concat','Category'])

    if(args.url is not None):          #Checking if the user provided with the url
        parsero(args.url,r)
        df_html=df_html.append({'Slno':0,'URL':args.url,'Length_of_document':html['Length_of_document'],
        ...
        df_js=df_js.append({'Slno':0,'URL':args.url,'Length_of_document':js['Length_of_document'],'No_of_iframes':js['No_of_iframes'],
    ...
    if(args.filename is not None):                                         #Checking if the user provided with the filename
        ...
        for ur in urls:
            ...
            ur=ur+'/'
            parsero(ur,r)
            df_html=df_html.append({'Slno':slno,'URL':ur,'Length_of_document':html['Length_of_document'],'No_of_iframes':html['No_of_iframes'],
            ...
            df_js=df_js.append({'Slno':slno,'URL':ur,'Length_of_document':js['Length_of_document'],'No_of_iframes':js['No_of_iframes'],
                           'Keywords':js['Keywords'],'escape':js['escape'],'unescape':js['unescape'],'link':js['link'],
                           'execo':js['execo'],'search':js['search'],'eval':js['eval'],'decode':js['decode'],
                           'encode':js['encode'],'concat':js['concat'],'Category':0},ignore_index=True)
